...
export const MapsPage: React.FC<MapsPageProps> = ({ setPage, user, darkMode }) => {
      ...
      cluster.schools.forEach(school => {
        ...
        const iconHtml = `
          <div style="display:flex;flex-direction:column;align-items:center;cursor:pointer;">
            ...
            <div style="
              margin-top:3px;
              background:${darkMode?'rgba(15,22,35,0.92)':'rgba(255,255,255,0.92)'};
              ...
              pointer-events:none;
            ">${school.name.length > 18 ? school.name.slice(0,18)+'…' : school.name}</div>
          </div>`;

        const icon = L.divIcon({ className:'', html:iconHtml, iconAnchor:[14,14] });
      ...
      const clusterHtml = `
        <div style="display:flex;flex-direction:column;align-items:center;cursor:pointer;">
          <div style="
            ...
            display:flex;align-items:center;justify-content:center;
          ">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#e85d04" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div style="
            ...
            pointer-events:none;
          ">${cluster.name.length>20?cluster.name.slice(0,20)+'…':cluster.name}</div>
        </div>`;

      const centerIcon = L.divIcon({className:'',html:clusterHtml,iconAnchor:[18,18]});
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in-up" style={{minHeight:0}}>
      ...
      <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-t-2xl px-4 py-2.5 space-y-2">
            ...
            <button key={layer.key} onClick={()=>toggleLayer(layer.key)}
              className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10.5px] font-bold transition-all border ${
                layers[layer.key]
                  ...
                  :'bg-slate-100 dark:bg-slate-800 text-slate-500 border-slate-200 dark:border-slate-700 opacity-50'
              }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 border border-slate-200 dark:border-slate-800 border-t-0 rounded-b-2xl overflow-hidden flex-1" style={{minHeight:'420px'}}>
        ...
        {/* MAP */}
        <div className="md:col-span-3 relative h-full w-full" style={{minHeight:'420px'}}>
          {loading&&(
