...
import { documentReportsApi, weeklyPlanningApi, BASE_URL } from '../api';
...
export const DocumentReportsPage: React.FC<DocumentReportsPageProps> = ({ user, showToast }) => {
  ...
  const [biweeklyReports, setBiweeklyReports] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      role === 'admin' ? weeklyPlanningApi.listReports() : Promise.resolve([]),
    ]).then(([inboxData, sentData, reportsData]) => {
      if (Array.isArray(inboxData)) setInbox(inboxData);
      if (Array.isArray(sentData))  setSent(sentData);
      if (Array.isArray(reportsData)) setBiweeklyReports(reportsData);
      setLoading(false);
    });
  }, [user]);
    ...
    <div>
      ...
      {tab === 'inbox' && (
        <div className="space-y-2">
          ...
          {role === 'admin' && biweeklyReports.length > 0 && (
            <div className="mb-4">
              <div className="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 mb-2">Biweekly Planning Reports</div>
              <div className="space-y-1.5">
                {biweeklyReports.map((r: any) => (
                  <button
                    key={r.id}
                    onClick={() => { window.location.href = `/weekly_planning_report?id=${r.id}`; }}
                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg border border-neutral-200 dark:border-slate-700 hover:border-[var(--brand-400)] text-left"
                  >
                    <div>
                      <div className="text-xs font-bold text-black dark:text-white">
                        Biweek starting {new Date(r.biweek_start_date).toLocaleDateString()}
                      </div>
                      <div className="text-[10px] text-slate-400">
                        Generated by {r.generated_by_name || 'Planning Officer'} on {new Date(r.created_at).toLocaleDateString()}
                      </div>
                    </div>
                    <FileText size={14} className="text-[var(--brand-500)] shrink-0" />
                  </button>
